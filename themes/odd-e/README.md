...
(`legacy/odde_training_template/template/odde_slides.tpl`): light grey
(#cdcdcd) background with the odd-e logo watermark, now top-right at 4%
scale (moved from the original top-left placement, which collided with
heading text once `styles/index.ts` gave headings proper sizing).
