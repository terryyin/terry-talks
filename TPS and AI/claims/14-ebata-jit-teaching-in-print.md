# Claim 14: Ebata's JIT teaching can be corroborated in print

**Status: Provisional — hypothesis stated; research not started**

## Original claim / hypothesis

> **Kazumasa Ebata's oral JIT teaching can be corroborated in published
> Japanese TPS sources, and should be attributed on stage as teaching plus
> Terry's notes, not as a Toyota quotation.**

Some of Terry's JIT learning comes from Kazumasa Ebata, whose father is a
TPS champion at Toyota. Ebata shared:

> **ジャストインタイム**
>
> 顧客の声に従う（事実 ← 体験、コラボーレーション）
>
> ある物で工夫する（事実 ← コラボーレーション）
>
> 早く提供する（提供者の希望 = 提供者の希望）
>
> 開発者の想像を破棄する（スキル = スキル）
>
> **大野耐一:** 余計なことはするな！

A working English rendering, using Terry's classifications: follow the
customer's voice (fact ← experience and collaboration); be resourceful with
what you have (fact ← collaboration); deliver quickly (a provider's hope);
discard the developer's imagination (skill, not speculation). Ohno: do not
do the unnecessary.

This is oral teaching, not a found canonical triad. Parenthetical notes are
Terry's. It was split from
[Claim 3](03-jidoka-enables-jit-trusts-respect-grows.md), which uses the
teaching as the closest Japanese-language resemblance to the JIT side of
Terry's mnemonic.

## Research-based adjustment

Not yet researched.

## Emerging implication for the talk

If published Japanese sources corroborate the teaching, the talk can present
it as grounded in TPS lineage rather than only as a colleague's oral
transmission. If they do not, keep it as Ebata's teaching and Terry's notes,
and do not imply Toyota publication.

## Questions still open

- Which published Japanese TPS, Ohno, or Toyota texts resemble these four
  JIT lines and Ohno's “余計なことはするな”?
- How close is the match: wording, structure, or only family resemblance?
- How should the parenthetical notes (fact, hope, skill) be attributed on
  stage so they are not heard as Ebata's or Toyota's?

## Sources consulted

1. Kazumasa Ebata, oral teaching shared with Terry. Not a published source.

**Research not started.**
