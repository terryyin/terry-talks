...
const WarpYarn: React.FC<{x: number; bulge: number}> = ({x, bulge}) => (
	<g>
		...
		/>
		<path
			d={vStroke(x + 1.2, 52, 658, bulge * 0.4)}
			stroke={INK}
			strokeWidth={0.6}
			fill="none"
			opacity={0.28}
	...
	return (
		<AbsoluteFill style={{backgroundColor: PAPER}}>
			<LoomSvg>
				...
				<path d="M 88 68 Q 800 50 1512 72" stroke={INK} strokeWidth={7} fill="none" opacity={0.4} strokeLinecap="round" />
				<path d="M 108 78 Q 800 62 1494 80" stroke={INK} strokeWidth={2.4} fill="none" opacity={0.18} strokeLinecap="round" />
				{WARPS.map((w) => (
					<path
						key={`guide-${w.x}`}
				...
				<path d="M 170 658 Q 800 638 1430 660" stroke={INK} strokeWidth={1.8} fill="none" opacity={0.35} strokeLinecap="round" />
				<path d="M 80 888 Q 800 872 1520 886" stroke={INK} strokeWidth={2.2} fill="none" opacity={0.22} strokeLinecap="round" />
			</LoomSvg>
			<AnimationEffect actor="stop-bar">
				<LoomSvg>
					...
					/>
					<path
						d="M 220 418 Q 800 406 1384 416"
						stroke="#efe8dc"
						strokeWidth={7}
						fill="none"
						opacity={0.22}
					...
					/>
					<path
						d="M 210 414 Q 800 400 1398 412"
						stroke={INK}
						strokeWidth={2.1}
						fill="none"
						opacity={0.55}
