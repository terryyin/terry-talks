...
<context>
Read `AGENTS.md`, `.cursor/rules/problem-decomposition.mdc`, and
...
2. Current `.planning/quick/` contents.
...
```bash
git log --all --name-status -- .planning/quick
git log --all --diff-filter=D --summary -- .planning/quick
...
normal `.planning/quick/` location. The PLAN must cite the original story and
reviewed commit set as its source. If the findings cannot form one bounded
outcome, let slice-planning's input gate stop rather than inventing an invalid
plan; report the required developer choice.
