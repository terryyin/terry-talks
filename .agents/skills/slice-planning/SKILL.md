...
name: slice-planning
description: >-
  Turn one selected story into an executable plan of stop-safe
  ...
  use slice-plan-refinement for a complex plan or an execution overrun. Use
  story-refinement for selected-story detail; use story-decomposition when the
  parent problem or candidate selection is unclear.
---
...
<locations>
- Ad-hoc executable plan: `.planning/quick/NNN-slug/PLAN.md`
- Never create `.planning/phases/` or other GSD files unless the owner asks.
</locations>
...
Do not end a slice with broken `pnpm typecheck` / `pnpm build`; leave checks
green for wrap-up. Keep product artifacts capability-named.
</step>
...
If a plan task bundles multiple Behaviors or speculative Structure, rewrite it
before execution. Do not implement feature or content work while planning.
</step>
...
- The response states direct-execution readiness or recommends refinement for
  named slices.
- The plan is under `.planning/quick/`.
...
`ready for direct execution` or `refinement recommended: <slices>`. This skill
only writes the PLAN; **execute-plan** runs the slices.
...
- Feature or content implementation.
- Direct execution of a story-decomposition seed.
- GSD files unless the owner asks.
</out_of_scope>
