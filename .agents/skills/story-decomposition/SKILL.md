...
Update a supplied seed or create `.planning/seeds/SEED-NNN-slug.md` with `NNN`
higher than the current maximum under `.planning/seeds/`. `slug` is kebab-case
from the parent problem. One seed represents the parent problem; its stories
are not separate seed files.

Preserve this seed frontmatter:
...
status: dormant
planted: YYYY-MM-DD
planted_during: <talk, claim, or other context>
trigger_when: <when this problem should surface>
scope: <small | medium | large | unknown for the whole set>
---
```

`planted_during` is talk, claim, or session context — not a GSD milestone or
phase. Do not run `/gsd-capture` or create GSD files.
...
- GSD capture (`/gsd-capture`) or GSD files (`PROJECT.md`, `REQUIREMENTS.md`,
  `ROADMAP.md`, `STATE.md`, `config.json`, `.planning/phases/`).
</out_of_scope>
